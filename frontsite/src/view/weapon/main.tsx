...
import { useWeaponList as useSkillList } from '../../state/skill/selector';
              ...
              { title: '覚醒',field: 'weaponIdBeforeLimitBreak', 
                ...
                editComponent:(props) => {
                  return <Select value={props.rowData.weaponIdBeforeLimitBreak} onChange={e => {props.onChange(e.target.value)}}>
                    ...
                    {
                      weaponList.filter((row) => row.name === props.rowData.name ).map((row) => <MenuItem key={row.id} value={row.id}>{row.name}</MenuItem>)
                    }
                    </Select>
                }
              } 
